<x-header>
    ...
    <div class="bigBox">
        <div class="w-full bg-white mt-5 px-16 h-8Y">
            <form autocomplete="off" action="/register" method="POST" class="w-8/12 text-center m-auto relative top-9">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-4 w-3/4 m-auto mt-4">
                ...
                <div class="grid grid-cols-1 w-3/4 m-auto mt-4">
                    <div class="flex flex-col">
                        <label class="text-start">E-Posta</label>
                        <input type="email" name="email" class="py-2 px-3 border mt-2" placeholder="E-posta giriniz"
                            value="{{ old('email') }}">
                        @error('email')
                            <span class="text-red-400 text-xs text-start mt-2">{{ $message }}</span>
                        @enderror
                ...
                <div class="grid grid-rows-2 gap-4 w-3/4 m-auto mt-4">
                    <div class="flex flex-col">
                ...
                <div class="w-2/3  m-auto mt-5 flex justify-between gap-4">
                    <button type="submit"
                        class="border border-blue-400 text-blue-400 hover:bg-blue-400 hover:text-white transition py-1 px-2">Kayıt
                        Ol</button>
                    <a href="{{ route('login') }}"
                        class="border border-red-400 text-red-400 hover:bg-red-400 hover:text-white transition py-1 px-2">Giriş
                        Yap</a>
                </div>
            </form>
        </div>
